...
#'
#' \code{racircalcsv} Corrects your RACiR data files based on a calibration file. Produces diagnostic graphs of A vs. Ci for quality control. Output includes a data frame in the global environment "corrected_data" and a csv file of the form "datafile.csv".
#'
#' @inheritParams racircalcheck
#' @param datafile Name of the data file to be corrected
#'
#' @return racircalcsv returns a data frame with corrected RACiR data
...
#' @export
#'
racircalcsv <- function(calfile, mincut, maxcut, datafile){
  # Load calibration data -----------------------------------
  cal <- read.csv(calfile)
  ...
  write.csv(id1, paste(datafile, "_corrected.csv", sep = ""))
}
